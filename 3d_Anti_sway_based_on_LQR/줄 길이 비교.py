...
class TaskManager:
    def __init__(self):
        ...
        self.phases = [
            ...
            ("MOVE_TO_TGT", *self.POS_END, self.L_LOW, 12.0),
            ("STABILIZE", *self.POS_END, self.L_LOW, 3.0),
            ("LOWER_TO_PLACE", *self.POS_END, self.L_LOW, 1.0),
            ("UNLATCHING", *self.POS_END, self.L_LOW, 1.5),      
            # 복귀 과정(HOIST_EMPTY, RETURN_HOME) 제거
            ("DONE", *self.POS_END, self.L_LOW, 0.0) 
        ]
...
ax3d.set_title("Crane Cycle: Pick -> Move -> Place (One Way)")
...
# [고속 렌더링 설정]
STEPS_PER_FRAME = 5 
# [수정됨] 작업이 빨리 끝나므로 전체 시간 단축 (60s -> 30s)
TOTAL_SIM_TIME = 30.0 
FPS = 15
...
anim.save("MK13_Simulation_OneWay.mp4", writer=writer, dpi=80) 
plt.close(fig)
...
# [수정됨] 최종 그래프에서도 Y축 한계 6000으로 고정
ax_f2.set_ylim(0, 6000)
...
fig_final.savefig("MK13_Full_Simulation_Graphs_OneWay.png", dpi=150)
plt.close(fig_final)

print(">>> 모든 저장 완료!")
print("1. 비디오: MK13_Simulation_OneWay.mp4")
print("2. 그래프: MK13_Full_Simulation_Graphs_OneWay.png")
