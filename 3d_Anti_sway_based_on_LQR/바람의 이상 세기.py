...
class TaskManager:
    def __init__(self):
        ...
        # [수정됨] 강풍 테스트를 위해 제한을 35.0으로 상향 (기존 22.0)
        self.WIND_LIMIT = 35.0
        self.is_danger_mode = False; self.frozen_state = None
...
# --- 4. 시각화 및 실행 ---
# [수정됨] 바람 범위를 15 ~ 30 m/s로 설정
sim = CranePhysicsRigidBody(); task = TaskManager()
wind = WindManager(min_speed=15.0, max_speed=30.0)
...
ax3d.set_title("Crane Cycle: Pick -> Move -> Place -> Return (HIGH WIND TEST)")
...
ax_sway.set_title("Sway & Wind"); ax_sway.set_ylim(0, 12); ax_sway.set_xlim(0, 10)
l_sway, = ax_sway.plot([], [], 'r-', lw=2, label='Sway')
l_wind, = ax_wind.plot([], [], 'b--', alpha=0.5, label='Wind')
ax_wind.set_ylim(0, 40); ax_wind.axhline(35, color='red', ls=':') # 그래프 축 범위도 40으로 증가
...
print(f"강풍(15~30m/s) 시뮬레이션 시작! (총 {TOTAL_FRAMES} 프레임)")
...
anim.save("MK13_Simulation_HighWind.mp4", writer=writer, dpi=80) 
plt.close(fig)
...
fig_final.suptitle(f"High Wind Simulation (15-30m/s) Result", fontsize=16, fontweight='bold')
...
ax_f3_wind.axhline(35, color='red', ls=':', alpha=0.5)
...
fig_final.savefig("MK13_Full_Simulation_HighWind_Graphs.png", dpi=150)
plt.close(fig_final)

print(">>> 모든 저장 완료!")
print("1. 비디오: MK13_Simulation_HighWind.mp4")
print("2. 그래프: MK13_Full_Simulation_HighWind_Graphs.png")
